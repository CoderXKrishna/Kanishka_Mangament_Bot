...
from KanishkaBot import DRAGONS
from KanishkaBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Kanishka_is_admin(chat_id: int):
    status = False
    Kanishka = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Kanishka.id == user.id:
            status = True
            break
    return status
