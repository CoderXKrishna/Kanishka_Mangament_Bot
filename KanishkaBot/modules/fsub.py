...
from KanishkaRobot import BOT_ID
from KanishkaRobot import DRAGONS as DEVS
from KanishkaRobot import OWNER_ID
from KanishkaRobot import telethn as Kanishka
from KanishkaRobot.events import Kanishkainline
from KanishkaRobot.events import register as Kanishkabot
from KanishkaRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Kanishka(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Kanishka(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Kanishkabot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
